      ...
      <el-table-column label="状态" align="center">
        <template slot-scope="scope">
          <el-switch
            ...
            active-value="0"
            inactive-value="1"
            @change="handleNoticeEnable(scope.row, scope.$index)"
          ></el-switch>
        </template>
      </el-table-column>
      <el-table-column
      ...
      updateNotice(params)
        .then((response) => {
          if (response.code === 200) {
            this.open = false;
            this.noticeList[index].status = status === "0" ? "0" : "1";
          }
        })
        .catch(() => {
          this.open = false;
          this.noticeList[index].status = status === "0" ? "1" : "0";
        });
    },
